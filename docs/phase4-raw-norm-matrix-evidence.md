...
**Verdict: on the IDENTICAL raw first batch, RLinf's FULL raw→quantile-normalized→loss path (canonical NEW
stats) reproduces the reference — raw hashes match, tokens/masks byte-identical, normalized state/actions
within `2.9e-8`, same-batch loss `0.4038` vs reference `0.4029` (`|Δ| = 0.0009 ≤ 0.01`). The per-batch
normalization APPLICATION is correct. So the significant ~9% eval gap (AC-3) is NOT a same-batch
transform / norm-stats-application bug — it is localized to the production training data TRAJECTORY, which a
single-batch test cannot exercise.**
...
So the significant ~9% eval gap between the single RLinf-trained and single reference-trained checkpoint is
**not** explained by any per-sample or per-batch code path. Per AC-4's positive test, a confirmed same-input
MATCH means the gap is the different training TRAJECTORY — the two are INDEPENDENT 30000-step SFT runs with
different data shuffle/coverage/RNG (the plan's "benign independent shuffle"). Establishing whether that
trajectory difference is a SYSTEMATIC RLinf disadvantage or run-to-run training variance would require
multiple independent RLinf-vs-reference training runs (as Phase-3's R24 did for the loss curve), which is
beyond AC-4's same-batch root-cause scope and the genuinely-new surfaces this plan targets. No localized
