...
pass-through for the FSDP manager. Everything else — observation construction,
action generation, training-mode forward — lives in the concrete subclasses:

* :class:`OpenPiPytorchEvalActionModel` — openpi.transforms pipeline +
  deterministic Euler ODE sampler (eval-only).
* :class:`OpenPiPytorchSFTActionModel` — vendored :class:`EvalProcessor` +
  flow-matching SFT loss.
* :class:`OpenPiPytorchRLActionModel` — inherits the eval path from
