...
def get_lr_scheduler(
    ...
    if lr_scheduler == "constant":
        ...
        # Reference-exact warmup+cosine (openpi-comet-pytorch-mixed): the warmup
        # starts at peak/(warmup+1) (not 0), ramps linearly to the peak at
        # `num_warmup_steps`, then cosine-decays to `min_lr` over the remaining
        # `num_training_steps - num_warmup_steps` steps. Returned as a multiplier
        # on the optimizer's base lr (the peak).
        from torch.optim.lr_scheduler import LambdaLR
