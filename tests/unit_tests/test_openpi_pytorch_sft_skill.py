...
def _make_full_dataset(use_skill, *, skill_labels=_LABELS):
    ...
    ds.episodes = [0]
    ds.meta = _MetaFull()
    ds.repo_id = "behavior-1k/2025-challenge-demos"
    ds.fine_grained_level = 0
...
def _full_item(frame, fps=30):
    return {
        "episode_index": _Scalar(0),
        "task_index": _Scalar(0),
        "timestamp": _Scalar(frame / fps),
    }
...
def test_set_prompt_uses_task_text_when_use_skill_false():
    # Even with skill_labels present, use_skill=False keeps the main-task prompt.
    ds = _make_full_dataset(use_skill=False, skill_labels=_LABELS)
    item = _full_item(250)
    ds._set_prompt(item)
    assert item["task"] == "turn on the radio"
    assert "prompt" not in item  # transform falls back to the task text
